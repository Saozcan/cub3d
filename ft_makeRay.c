...
/* 	i = data->thirdAngle[1];
	...
	while (i++ < data->fourthAngle[0] && data->fourthAngle[0] != data->fourthAngle[1]) {
		k = 0;
		while (k <= 140) {
			checkPozY1 = ((data->x1 + (70 - (int)data->x1 % 70) + k) * (1/(tan(data->val * i))));
			checkPozX1 = data->x1 + k + (70 - ((int)data->y1) % 70);
			printf("x: %f y: %f\n", data->x1, data->y1);
			printf("y1: %f, nY1: %d, x1: %f, nX1: %d \n", \
			(data->x1 + (70 - (int)data->x1 % 70) + k), \
			checkPozY1, data->x1 + k + (70 - ((int)data->y1) % 70), checkPozX1);
			printf("firstAnlge: %f, secAnlge: %f\n", data->fourthAngle[0], data->fourthAngle[1]);
			mlx_pixel_put(data->mlx_ptr, data->mlx_win, checkPozX1, checkPozY1, 0x00ff00);
			/* if (wall_check(* data, checkPozX, checkPozY)) {
				break ;
			} */
